...
def GenerateSampleFeature(InteractionList, EmbeddingFeature1, EmbeddingFeature2):
    ...
    SampleFeature1 = SampleFeature1.reshape(SampleFeature1.shape[0], SampleFeature1.shape[1], 1)
    SampleFeature2 = SampleFeature2.reshape(SampleFeature2.shape[0], SampleFeature2.shape[1], 1)
...
def GenerateBehaviorFeature_01(InteractionPair, NodeBehavior):
    SampleFeature1 = []
    for i in range(len(InteractionPair)):
        Pair1 = InteractionPair[i][0]
...
def GenerateBehaviorFeature_02(InteractionPair, NodeBehavior):
    SampleFeature1 = []
    for i in range(len(InteractionPair)):
        Pair1 = InteractionPair[i][1]
...
if __name__ == '__main__':
    MiRembedding = []
    ReadMyCsv3(MiRembedding, 'miSampleFeature.csv')
    CicRembedding = []
    ReadMyCsv3(CicRembedding, 'circSampleFeature.csv')
    AllNodeBehavior = []
    ReadMyCsv1(AllNodeBehavior, 'HOPE_128_Behavior.csv')
    ...
    x_train_1_Attribute = GenerateBehaviorFeature_02(x_train_pair, CicRembedding)
    x_train_2_Attribute = GenerateBehaviorFeature_01(x_train_pair, MiRembedding)
    x_validation_1_Attribute = GenerateBehaviorFeature_02(x_validation_pair, CicRembedding)
    x_validation_2_Attribute = GenerateBehaviorFeature_01(x_validation_pair, MiRembedding)
    x_test_1_Attribute = GenerateBehaviorFeature_02(x_test_pair, CicRembedding)
    x_test_2_Attribute = GenerateBehaviorFeature_01(x_test_pair, MiRembedding)
 ...
 # ———————————————————— 5 times —————————————————————
    ...
    while CounterT < 5:
        ...
        hidden = Dense(64, activation='relu', name='hidden2', activity_regularizer=regularizers.l2(0.001))(flatten)
